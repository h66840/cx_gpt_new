...
export const chatApi = {
   ...
   */
  getAgentHistory: (agentName, threadId) =>
    apiGet(`/api/chat/agent/${agentName}/history?thread_id=${threadId}`, {}, true),
...
// 注册、初始化、检查首次运行等，这些API通常不需要Token，因为用户还未登录或首次设置。
export const authApi = {
  /**
   * 检查系统是否首次运行。
   * @returns {Promise<{first_run: boolean}>} - 首次运行状态
   */
  checkFirstRun: () => apiGet('/api/auth/check-first-run', {}, false), // 不需要认证

  /**
   * 初始化超级管理员账户。
   * @param {Object} adminData - 管理员数据 (username, password)
   * @returns {Promise<Object>} - 初始化结果
   */
  initialize: (adminData) => apiPost('/api/auth/initialize', adminData, {}, false), // 不需要认证

  /**
   * 用户注册。
   * @param {Object} userData - 用户注册数据 (username, password, organization)
   * @returns {Promise<Object>} - 注册结果
   */
  register: (userData) => apiPost('/api/auth/register', userData, {}, false) // 不需要认证
}
