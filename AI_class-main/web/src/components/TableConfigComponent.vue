    ...
    <a-form layout="vertical">
      <div v-for="(item, index) in configList" :key="index" class="config-item">
        <a-row :gutter="[16, 8]" align="middle">
          <a-col :span="8">
            <a-input v-model:value="item.key" placeholder="模型名称" readonly class="key-input" />
          </a-col>
          <a-col :span="14">
            <a-input
    ...
    >
      <a-form layout="vertical">
        <a-form-item label="模型名称（与Huggingface名称一致，比如 BAAI/bge-large-zh-v1.5" required>
          <a-input v-model:value="newConfig.key" placeholder="请输入模型名称" class="modal-input" />
        </a-form-item>
        <a-form-item
          label="模型本地路径（绝对路径，比如 /hdd/models/BAAI/bge-large-zh-v1.5）"
          required
        >
          <a-input
