<template>
  <div
    class="chat"
    ref="chatContainer"
    :class="{ 'refs-sidebar-open': refsSidebarVisible && refsSidebarPinned }"
  >
...
const isLatestMessage = (index) => {
  // 找到最后一条助手消息的索引
  const lastAssistantMsgIndex = findLastIndex(
    conv.value.messages,
...
.chat {
  ...
  --chat-gradient: linear-gradient(135deg, #1677ff, #06b6d4);
...
.chat {
  ...
  &.refs-sidebar-open {
    padding-right: var(--refs-sidebar-pinned-width);
    /* 与侧边栏固定时的宽度一致 */
  }
