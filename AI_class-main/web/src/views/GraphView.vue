<template>
  ...
  <div class="graph-container layout-container" v-else>
    <HeaderComponent title="图数据库" :description="graphDescription">
      <template #actions>
    ...
    >
      <div v-if="graphInfo?.embed_model_name">
        ...
        />
        <p>
          当前图数据库向量模型：{{ graphInfo?.embed_model_name }}， 当前所选择的向量模型是
          {{ cur_embed_model }}
        </p>
      </div>
      ...
      <div class="upload">
        ...
        >
          ...
          <p class="ant-upload-hint">目前仅支持上传 jsonl 文件。且同名文件无法重复添加。</p>
        </a-upload-dragger>
      </div>
    </a-modal>
  </div>
...
const loadSampleNodes = () => {
  state.fetching = true
  graphApi
    .getNodes('neo4j', sampleNodeCount.value)
    .then((data) => {
...
const onSearch = () => {
  ...
  if (graphInfo?.value?.embed_model_name !== cur_embed_model.value) {
    ...
    if (
      !confirm(
        `构建图数据库时向量模型为 ${graphInfo?.value?.embed_model_name}，当前向量模型为 ${cur_embed_model.value}，是否继续查询？`
      )
    ) {
      return
    }
